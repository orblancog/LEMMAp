...
int DrawPhaseSpace1D (const char * k, const int giro=-1) {//k is the flag name , giro is the turn number
  ...
  trackfl->Append(k);
  TH2 * trackpxd = new TH2F("Phase Space PX,PT","Phase Space PX,PT",81,-4,4,101,-100,-100);//number,-x,x,number,-y,y
  ...
  while(!track0in.eof()){
    ...
    if (giro == atof(track02) || giro == -1){
      trackpxd->Fill(atof(track04)*scalehv,atof(track08)*scalehv);
  ...
  gPad->SetLogz();
  trackpxd->Draw("colz");
  trackpxd->SetTitle("Phase Space PX vs E");
  trackpxd->GetXaxis()->SetTitle("px/p0 [mrad]");
  trackpxd->GetYaxis()->SetTitle("e.spread/(p0*clight) [1e-3]");
  trackpxd->GetXaxis()->CenterTitle();
  trackpxd->GetYaxis()->CenterTitle();
  c1->cd(2);
  trackpxd->ProjectionX()->Draw();
  c1->cd(3);
  trackpxd->ProjectionY()->Draw();
  // c1->cd(2);
  // gPad->SetLogz();
  // trackypy->Draw("colz");
